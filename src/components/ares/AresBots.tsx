...
function analyzeConfluence(candles: MarketCandle[], quote: MarketQuote, response: CandleResponse, risk: BotRisk): Analysis {
  ...
  const stop = signalDirection >= 0 ? Math.max(support ?? last * (1 - riskFactor), last * (1 - riskFactor)) : Math.min(resistance ?? last * (1 + riskFactor), last * (1 + riskFactor));
  const target = signalDirection >= 0 ? Math.min(resistance ?? last * (1 + riskFactor * 2), last * (1 + riskFactor * 2.2)) : Math.max(support ?? last * (1 - riskFactor * 2), last * (1 - riskFactor * 2.2));
  ...
  const invalidation = signalDirection >= 0 ? `La lectura pierde validez si el precio rompe el soporte ${formatValue(support)} o MACD y volumen dejan de confirmar.` : `La lectura pierde validez si el precio supera la resistencia ${formatValue(resistance)} o MACD y volumen dejan de confirmar.`;
...
export function AresBots() {
  ...
  return <div className="space-y-5">
    ...
    <section className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-6" aria-label="Acciones del bot"><Button onClick={analyze} disabled={loading} className="h-12 rounded-xl bg-blue-600 text-xs font-black hover:bg-blue-700">{loading ? <LoaderCircle className="mr-2 size-4 animate-spin"/> : <Zap className="mr-2 size-4"/>}ANALIZAR</Button><Button onClick={() => setConfigOpen((value) => !value)} variant="outline" className="h-12 rounded-xl border-slate-200 text-xs font-black"><Settings2 className="mr-2 size-4"/>CONFIGURAR BOT</Button><Button onClick={simulate} variant="outline" className="h-12 rounded-xl border-violet-200 bg-violet-50 text-xs font-black text-violet-700"><Play className="mr-2 size-4"/>SIMULAR</Button><Button onClick={save} variant="outline" className="h-12 rounded-xl border-emerald-200 bg-emerald-50 text-xs font-black text-emerald-700"><Save className="mr-2 size-4"/>GUARDAR<span className="hidden 2xl:inline"> CONFIGURACIÓN</span></Button><Button onClick={reset} variant="outline" className="h-12 rounded-xl border-slate-200 text-xs font-black"><RefreshCw className="mr-2 size-4"/>REINICIAR</Button><Button onClick={() => helpRef.current?.scrollIntoView({ behavior: "smooth" })} variant="outline" className="h-12 rounded-xl border-amber-200 bg-amber-50 text-xs font-black text-amber-800"><CircleHelp className="mr-2 size-4"/>AYUDA</Button></section>
