...
function metrics(points: Point[], intraday: number | null): Metrics {
  ...
  const bySessions=(sessions:number)=>values.length>sessions?pct(last,values[values.length-1-sessions]):null;
  const byCalendarDays=(days:number)=>{const target=lastTime-days*86400000;const prior=points.filter(point=>Date.parse(point.time)<=target).at(-1);const next=points.find(point=>Date.parse(point.time)>=target);const candidate=!prior?next:!next?prior:target-Date.parse(prior.time)<=Date.parse(next.time)-target?prior:next;return candidate&&Math.abs(Date.parse(candidate.time)-target)<=4*86400000?pct(last,candidate.value):null;};
  ...
  return {returns:{intraday,week:bySessions(5),tenDays:byCalendarDays(10),twentyDays:byCalendarDays(20),thirtyDays:byCalendarDays(30),month:byCalendarDays(30),threeMonths:byCalendarDays(90),sixMonths:byCalendarDays(180),year:byCalendarDays(365)},ema9:e9,ema21:e21,emaSlope,rsi14:rsi(values),macd,macdSignal:signal,macdHistogram:hist,bollingerZ:z,bollingerExpansion:bandExpansion,volumeRatio,volumeTrend,support,resistance,breakoutPct,failedBreakout,volatility:vol,trend,samples:values.length};
}
